-- ==========================================
-- 028_activity_sessions.sql
-- Timer-based Activity Sessions with Secure Validation
-- ==========================================
...
CREATE TABLE IF NOT EXISTS public.activity_sessions (
    ...
    activity_type VARCHAR(50) NOT NULL, -- 'breathing', 'exercise', 'meditation', etc.
    start_time TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT NOW(),
    duration_seconds INTEGER NOT NULL CHECK (duration_seconds > 0),
    actual_duration_seconds INTEGER, -- Actual time spent when completed
...
-- Policy for guests to manage their sessions
...
-- Secure Timer Completion Function
CREATE OR REPLACE FUNCTION complete_activity_session(
    p_session_id UUID,
...
BEGIN
    ...
    ELSIF p_guest_session_id IS NOT NULL THEN
        ...
        END IF;
    ELSE
        RETURN QUERY SELECT FALSE, 'Invalid authentication'::TEXT, NULL::UUID;
        RETURN;
    END IF;
    ...
    -- Validate duration
    IF v_elapsed_seconds < v_session.duration_seconds THEN
        RETURN QUERY SELECT FALSE, 
            FORMAT('Timer not completed yet. %s seconds remaining', 
                   v_session.duration_seconds - v_elapsed_seconds)::TEXT, 
            v_session.id;
        RETURN;
    END IF;
...
GRANT EXECUTE ON FUNCTION complete_activity_session TO authenticated;
GRANT EXECUTE ON FUNCTION complete_activity_session TO anon;

-- Helper function to start a new activity session
...
BEGIN
    ...
    IF p_user_id IS NULL AND p_guest_session_id IS NULL THEN
        RETURN QUERY SELECT FALSE, 'User authentication required'::TEXT, NULL::UUID;
        RETURN;
    END IF;
...
-- Grant execute permission to authenticated users
GRANT EXECUTE ON FUNCTION start_activity_session TO authenticated;
GRANT EXECUTE ON FUNCTION start_activity_session TO anon;
