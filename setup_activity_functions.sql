...
BEGIN
    ...
    -- Additional safety check: if elapsed time is very close to target, allow completion
    IF v_elapsed_seconds >= (v_session.duration_seconds * 0.98) THEN  -- Allow if 98% complete
        NULL;  -- Continue to completion
    ELSIF v_elapsed_seconds < v_session.duration_seconds THEN
        RETURN QUERY SELECT FALSE, 
            FORMAT('Timer not completed yet. %s seconds remaining', 
